...
// Giriş kontrolü
if (!$current_user) {
    echo '<script>window.location.href = "/login?redirect=/dashboard";</script>';
    exit;
}
?>
...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            ");
                $stmt->execute([$customer_id]);
                $student_support = $stmt->fetchColumn() ?: 0;
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">₺<?php echo number_format($student_support, 2); ?>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <?php if ($current_user['role'] === 'student'):
                ...
                // Toplam ürün sayısı
                $stmt = $db->prepare("SELECT COUNT(*) FROM products WHERE student_id = ?");
                $stmt->execute([$student_id]);
                $total_products = $stmt->fetchColumn();


                // Onaylanan ürünler (active)
                $stmt = $db->prepare("SELECT COUNT(*) FROM products WHERE student_id = ? AND status = 'active'");
                $stmt->execute([$student_id]);
                $approved_products = $stmt->fetchColumn();

                // Bekleyen ürünler (pending)
                $stmt = $db->prepare("SELECT COUNT(*) FROM products WHERE student_id = ? AND status = 'pending'");
                $stmt->execute([$student_id]);
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">₺<?php echo number_format($total_earnings, 2); ?>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <?php if ($current_user['role'] === 'merchant'):
                ...
                $shop_stmt = $db->prepare("SELECT id FROM shops WHERE owner_id = ?");
                $shop_stmt->execute([$merchant_id]);
                $shop = $shop_stmt->fetch();
                $shop_id = $shop ? $shop['id'] : 0;

                $stmt = $db->prepare("SELECT COUNT(*) FROM products WHERE shop_id = ?");
                $stmt->execute([$shop_id]);
                $total_products_m = $stmt->fetchColumn();

                $stmt = $db->prepare("SELECT COUNT(*) FROM orders o INNER JOIN order_items oi ON o.id = oi.order_id INNER JOIN products p ON oi.product_id = p.id WHERE p.shop_id = ?");
                $stmt->execute([$shop_id]);
                $total_orders_m = $stmt->fetchColumn();


                $stmt = $db->prepare("SELECT COUNT(*) FROM products WHERE shop_id = ? AND status = 'pending'");
                $stmt->execute([$shop_id]);
                $pending_approvals = $stmt->fetchColumn();

                $stmt = $db->prepare("SELECT SUM(o.total_amount) FROM orders o INNER JOIN order_items oi ON o.id = oi.order_id INNER JOIN products p ON oi.product_id = p.id WHERE p.shop_id = ? AND o.status = 'delivered'");
                $stmt->execute([$shop_id]);
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="space-y-4">
                    ...
                    if ($current_user['role'] === 'student') {
                        ...
                        // Merchant aktiviteleri: Mağazadaki son siparişler
                        $shop_stmt_act = $db->prepare("SELECT id FROM shops WHERE owner_id = ?");
                        $shop_stmt_act->execute([$user_id]);
                        $shop_act = $shop_stmt_act->fetch();
                        $shop_id_act = $shop_act ? $shop_act['id'] : 0;

                        $stmt = $db->prepare("
                            SELECT 'order' as type, o.order_number as message, o.created_at, o.status
                            FROM orders o
                        ...
                        foreach ($orders as $order) {
                            ...
                            $activities[] = [
                                'type' => 'order',
                                'message' => $order['message'] . ' siparişi ' . $status_text,
                                'time' => $order['created_at'],
                                'icon' => 'shopping-bag'
                            ];
                        }
                    ...
                    function timeAgo($datetime)
                    {
                        ...
                        return date('d', $time) . ' ' . $aylar[(int) date('n', $time)] . ' ' . date('Y', $time);
                    }
                    ...
                    <?php else:
                        foreach ($activities as $activity):
                            ...
                            <div class="flex items-center space-x-3">
                                <div
                                    class="w-8 h-8 <?php echo $bg_class; ?> rounded-full flex items-center justify-center flex-shrink-0">
                                    <svg class="w-4 h-4 <?php echo $icon_class; ?>" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
...
<script>
    // Dashboard verileri PHP tarafından server-side render ediliyor.
    // JavaScript ile dinamik veri yükleme gerektiğinde buraya eklenecek.
</script>